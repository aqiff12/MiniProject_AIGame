...
def custom_score(game, player):
    ...
    myMoveList = game.get_legal_moves(player)
    oppMoveList = game.get_legal_moves(game.get_opponent(player))

    for i in myMoveList:
        game.forecast_move(i)
        my_moves += len(game.get_legal_moves())

    for j in oppMoveList:
        game.forecast_move(j)
        opponent_moves += len(game.get_legal_moves())
    calc = my_moves - (opponent_moves * 2) + len(myMoveList)
    return calc   
    # for i in myMoveList:
    #     print(i)
    #     for j in oppMoveList:
    ...
    #         else:
    #             print(betul)
    #         if myMoveList[i][j] == oppMoveList[i][j]:
    #             calc = myMoveList[i][j] - (oppMoveList[i][j] * 2) + my_moves
    #             return calc
    #         else:
    #             return i - (j * 2)
