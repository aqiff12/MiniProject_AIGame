...
class Frog(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.rect.y <= 175 and self.rect.y != 50 and self.dead == False:
            ...
            if onLog == False:
                self.die()
        elif self.rect.y == 200 and Population.isHalfFinished == False:
            self.fitness = 6
            Population.isHalfFinished == True
...
class Population:
    ...
    def randomize(self):
        for i in range(0, self.alive):
            ...
            if(self.isHalfFinished == True):
                frogs.add(Frog(167.5, 200, self.size, dir))
            else:
    ...
    def selection(self):
        bestHalf = list(self.bestHalfFrogDirections())
        best = list(self.bestFrogDirections())

        newFrogs = []
        if (self.isHalfFinished == False):
            dir = list(bestHalf)
            ...
            for frog in newFrogs:
                frogs.add(frog)
        elif (self.isFinished == False and self.isHalfFinished==True):
            dir = list(best)
            newDirections = FrogDirections(1000, dir)
            # Save the best frog
            newFrogs.append(Frog(167.5, 200, self.size, newDirections))
            ...
            frogs.empty()
            # final = []
            # final.extend(bestHalf)
            # final.extend(best)
            # print(final)

            print("BEST: ", best)
            print("================")
            print("BEST HALF: ", bestHalf)
    ...
    def bestHalfFrogDirections(self):
        print(self.isHalfFinished)
        if (self.isHalfFinished == False):
            # print("HALFINISHED")
            ...
            for i in range(0, frogNum - 1):
                if sortedFrogs[i].brain.step < sortedFrogs[frogNum - 1].brain.step and sortedFrogs[i].fitness == sortedFrogs[frogNum - 1].fitness:
                    best = i
            if (sortedFrogs[best].fitness >= 6):
                self.isHalfFinished = True
                Frog.fitness = 7
            else:
                self.generation += 1
            ...
            for frog in frogs:
                if (sortedFrogs[best].fitness == frog.fitness and sortedFrogs[best].brain.step == frog.brain.step):
                    bestHalfFrogDirections = list(frog.brain.directions)
                    break
            return bestHalfFrogDirections
        else:
            for frog in frogs:
                bestHalfFrogDirections = list(frog.brain.directions)
            # print("BESt")
...
def set():
    ...
    for i in range(0, 9):
        if i < 3:
            all_sprites.add(
                Log(-100 + 150 * (3 - i), 150, 'short', 62.5, 25, 3))
        elif i < 6:
            all_sprites.add(Log(-150 + 200 * (6 - i), 125, 'long', 150, 25, 4))
        else:
            all_sprites.add(
                Log(-200 + 150 * (9 - i), 75, 'medium', 87.5, 25, 6))
    for i in range(0, 12):
        if i < 3:
            all_sprites.add(
                Car(100 + 75 * (3 - i), 325, 'yellow', 6, 1, 25, 25))
        elif i < 6:
            all_sprites.add(
                Car(-150 + 75 * (6 - i), 300, 'dozer', 2, -1, 25, 25))
        elif i < 9:
            all_sprites.add(
                Car(50 + 75 * (9 - i), 275, 'purple', 4, 1, 25, 25))
        elif i < 10:
            all_sprites.add(
                Car(25 + 75 * (10 - i), 250, 'green', 10, -1, 25, 25))
        else:
            all_sprites.add(
                Car(50 + 150 * (12 - i), 225, 'truck', 3, 1, 50, 25))


# (number of alive frogs, number of directions)
...
while not finish:
    ...
    if (Population.frogsAlive == 0):
        pop.selection()
        set()
        # Sleeps so that the frogs are given a chance to reset properly before the game restarts
        time.sleep(2.5)
